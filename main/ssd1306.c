...
void ssd1306_display_text(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	...
	for (uint8_t i = 0; i < _text_len; i++) {
		...
		i2cdev_display_image(dev, page, seg, image, 8);
		seg = seg + 8;
	}
}
...
void ssd1306_display_image(SSD1306_t * dev, int page, int seg, uint8_t * images, int width)
{
	...
	i2cdev_display_image(dev, page, seg, images, width);
...
void ssd1306_scroll_text(SSD1306_t * dev, char * text, int text_len, bool invert)
{
	...
	while(1) {
		...
		if (dev->_page[dstIndex]._valid) i2cdev_display_image(dev, dstIndex, 0, dev->_page[dstIndex]._segs, dev->_page[srcIndex]._segLen);
	...
	for (uint8_t i = 0; i < _text_len; i++) {
		memcpy(image, font8x8_basic_tr[(uint8_t)text[i]], 8);
		if (invert) ssd1306_invert(image, 8);
		i2cdev_display_image(dev, srcIndex, seg, image, 8);
...
{
		i2cdev_hardware_scroll(dev, scroll);
}
...
void ssd1306_fadeout(SSD1306_t * dev)
{
	...
	for(int page=0; page<dev->_pages; page++) {
			...
			for(int seg=0; seg<128; seg++) {
				i2cdev_display_image(dev, page, seg, image, 1);
			}
		}
	}
}
